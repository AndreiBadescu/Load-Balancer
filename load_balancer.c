...
int binary_search(const load_balancer* main, const unsigned int hash) {
    ...
    while (left <= right) {
        int mid = (left + right) / 2; // can't overflow because MAX SIZE is 3e+5
...
void loader_add_server(load_balancer* main, int server_id) {
    ...
    for (int i = 0; i < MAX_SERVERS; ++i) {
        ...
        memmove(main->hashring + ind + 1, main->hashring + ind, (main->size - ind) * sizeof(main->hashring[0]));
...
void loader_remove_server(load_balancer* main, int server_id) {
    ...
    for (int cnt = 0, i = main->size - 1; i >= 0; --i) {
        if (main->hashring[i].server_id == server_id) {
            ...
            memmove(main->hashring + i, main->hashring + i + 1, (main->size - i - 1) * sizeof(main->hashring[0]));
            --main->size;
    ...
    for (int i = 0; i < CAPACITY; ++i) {
        for (ht_node* head = memory_ptr->buckets[i]; head; head = head->next) {
            ...
            if (ind == main->size) { ind = 0; }
            server_store(main->hashring[ind].hashtable, head->item->key, head->item->value);
        }
    }
...
void free_load_balancer(load_balancer* main) {
    ...
    DIE(is_deleted == NULL, "free load balancer error, calloc call returned NULL");
